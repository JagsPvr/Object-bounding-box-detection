...
class Model(object):
	...
	def get_model(self):
		...
		#add model layers
		model.add(Conv2D(32, kernel_size=3, activation=LR, input_shape=(int(480*H.img_scale_factor),int(640*H.img_scale_factor),3)))
		# model.add(Conv2D(64, kernel_size=5, activation='relu'))
		model.add(MaxPooling2D(pool_size = (2,2)))
		model.add(Conv2D(64, kernel_size=5, activation=LR))
		# model.add(Conv2D(64, kernel_size=7, activation='relu'))		
		model.add(MaxPooling2D(pool_size = (2,2)))
		model.add(Conv2D(128, kernel_size=5, activation=LR))
		# model.add(Conv2D(64, kernel_size=7, activation='relu'))		
		model.add(MaxPooling2D(pool_size = (2,2)))
		model.add(Conv2D(64, kernel_size=5, activation=LR))
		# model.add(Conv2D(64, kernel_size=7, activation='relu'))		
